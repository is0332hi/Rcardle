...
public class maincharacterspecial2 : MonoBehaviour {
    ...
    void Update()
    {
        if (readyprocess.maincharacternum == 2)
        {
    ...
    void OnMouseDown()
    {
        if (scenemanager.nextsection == scenemanager.NextSection.SELECT)
        {
            if (maincharacter.Specialcount != 0)
            {
                ...
                scenemanager.maincharacterpreactionflag = 1;
    ...
    void OnMouseExit()
    {
        if (scenemanager.nextsection == scenemanager.NextSection.SELECT)
        {
            cslarge2.descriptionflag = false;
            transform.localScale = new Vector3(0.75f, 0.8f);
        }
    }
